...
class ArbitrageScout(ScriptStrategyBase):
    ...
    # 过滤不想监控的交易对
    trading_pairs_set = [
        word for word in trading_pairs_set if all(letter not in word for letter in ["-ETH", "-BTC", "-TRY", "5", "3"])
    ]
    ...
    def on_tick(self):
        ...
        self.notify_hb_app_with_timestamp(
            f"Exchange_1: {self.arbitrage_markets[0]}; Exchange_2: {self.arbitrage_markets[1]}"
        )
